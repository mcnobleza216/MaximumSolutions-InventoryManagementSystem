...
Public Class stock_monitor
    ...
    Private Sub stock_monitor_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        depreciateitems()
        strcon.Close()
    ...
    Public Sub GetDateDiff()
        Try
            ...
            '    updates("UPDATE tbl_stockmonitorvalue SET depreciation_value = '" & txtb_depreciatedvalue.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
    ...
    Public Sub getunitcondition()
        Try
            strcon.Open()
            cmd.CommandText = "SELECT unit_condition from tbl_monitorcondition WHERE serial_number = '" & txtbx_serialno.Text & "'"
            cmd.Connection = strcon
            ...
            Else
                ...
                If dr.HasRows Then
                    ...
                    Else
                        ...
                        cmdd.CommandText = "SELECT depreciation_value FROM tbl_stockmonitorvalue WHERE serial_number = '" & txtbx_serialno.Text & "'"
    ...
    Private Sub txtbx_addunit_TextChanged(sender As Object, e As EventArgs) Handles txtbx_addunit.TextChanged
        ...
        If txtbx_addunit.Text.Length >= 19 Then
            Try
    ...
    Public Sub AddUnit()
        Try
            ...
            Else
                ...
                GetDateDiff()
                strcon.Close()
                create("INSERT INTO tbl_stockmonitor (unit_id,serial_number,brand_name,model,purchased_value,date_purchased) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','" & txtbx_model.Text & "','" & txtb_purchasedvalue.Text & "','" & dtp_purchased.Text & "')")
                strcon.Close()
                ...
                strcon.Close()
                create("INSERT INTO tbl_allstocks (unit_id,serial_number,brand_name,model,unit_type,unit_condition,availability,date_added) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','" & txtbx_model.Text & "','Monitor','Working','Available','" & lbl_datetoday.Text & "')")
                strcon.Close()
                create("INSERT INTO tbl_monitorcondition (unit_id,serial_number,brand,model,unit_condition) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "','" & cmb_brand.Text & "','" & txtbx_model.Text & "', 'Working')")
                strcon.Close()
                create("INSERT INTO tbl_inventorystorage (unit_id,serial_number,brand_name,model,unit_type,unit_condition) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "','" & cmb_brand.Text & "','" & txtbx_model.Text & "','Monitor','Working')")
                strcon.Close()
                MessageBox.Show("Monitor added sucessfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                reload("SELECT * FROM tbl_stockmonitor", dgv_stockunit)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
    ...
    Public Sub FilterData(valueToSearch As String)
        ...
        If cmb_searchfilter.Text = "SERIAL NO." Then
            Dim searchquery As String = "SELECT * FROM tbl_stockmonitor WHERE CONCAT(serial_number) LIKE '%" & txtbx_search.Text & "%'"
        ...
        ElseIf cmb_searchfilter.Text = "BRAND" Then
            Dim searchquery As String = "SELECT * FROM tbl_stockmonitor WHERE CONCAT(brand_name) LIKE '%" & txtbx_search.Text & "%'"
            Dim command As New MySqlCommand(searchquery, strcon)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable
        ...
        ElseIf cmb_searchfilter.Text = "MODEL" Then
            Dim searchquery As String = "SELECT * FROM tbl_stockmonitor WHERE CONCAT(model) LIKE '%" & txtbx_search.Text & "%'"
            Dim command As New MySqlCommand(searchquery, strcon)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable
