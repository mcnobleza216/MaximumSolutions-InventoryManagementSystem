...
Public Class stock_networkdevice
    ...
    Private Sub stock_networkdevice_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        depreciateitems()
        strcon.Close()
        reload("SELECT * FROM tbl_stocknetworkdevice", dgv_stockunit)
        FilterData("")
        ...
        Try
            ...
            cmd3.CommandText = "SELECT * FROM tbl_networkdevicetype"
    ...
    Private Sub Btn_Add2_Click(sender As Object, e As EventArgs) Handles Btn_Add2.Click
        ...
        If (lbl_unitid.Text = "" Or txtbx_addunit.Text = "" Or txtbx_model.Text = "" Or cmb_brand.Text = "" Or txtb_purchasedvalue.Text = "" Or dtp_purchased.Text = "" Or cmb_peripheraltype.Text = "") Then
    ...
    Public Sub ClearText()
        ...
        ' txtbx_serialno.Text = ""
        txtbx_model.Text = ""
        cmb_peripheraltype.Text = "Please Select"
    ...
    Public Sub getunitcondition()
        Try
            strcon.Open()
            cmd.CommandText = "SELECT unit_condition from tbl_peripheralcondition WHERE serial_number = '" & txtbx_serialno.Text & "'"
            cmd.Connection = strcon
        ...
        Try
            ...
            Else
                ...
                If dr.HasRows Then
                    ...
                    txtb_purchasedvalue.Text = dr.GetValue(5).ToString()
                    '  dtp_purchased.Text = dr.GetValue(6).ToString()
                    ...
                    Else
                        ...
                        cmdd.CommandText = "SELECT depreciation_value FROM tbl_stocknetworkdevicevalue WHERE serial_number = '" & txtbx_serialno.Text & "'"
    ...
    '        If (lbl_unitid.Text = "-------" Or txtbx_serialno.Text = "" Or txtbx_model.Text = "" Or cmb_brand.Text = "" Or txtb_purchasedvalue.Text = "" Or dtp_purchased.Text = "" Or cmb_peripheraltype.Text = "") Then
    '            MessageBox.Show("All information must be provided!", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Warning)
    '        Else
    '            UpdateUnit()
    '            txtbx_addunit.Text = ""
    ...
    Public Sub AddUnit()
        Try
            ...
            Else
                ...
                GetDateDiff()
                strcon.Close()
                create("INSERT INTO tbl_stocknetworkdevice (unit_id,serial_number,brand_name,model,type,purchased_value,date_purchased) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','" & txtbx_model.Text & "','" & cmb_peripheraltype.Text & "','" & txtb_purchasedvalue.Text & "','" & dtp_purchased.Text & "')")
                strcon.Close()
                create("INSERT INTO tbl_allstocksvalue (unit_id,serial_number,brand_name,model,type,purchased_value,depreciation_value,date_purchased) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','" & txtbx_model.Text & "','" & cmb_peripheraltype.Text & "','" & txtb_purchasedvalue.Text & "','" & txtb_depreciatedvalue.Text & "','" & dtp_purchased.Text & "')")
                strcon.Close()
                create("INSERT INTO tbl_allstocks (unit_id,serial_number,brand_name,model,unit_type,unit_condition,availability,date_added) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','" & txtbx_model.Text & "','Networking','Working','Available','" & lbl_datetoday.Text & "')")
                strcon.Close()
                create("INSERT INTO tbl_networkdevicecondition (unit_id,serial_number,brand,model,type,unit_condition) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "','" & cmb_brand.Text & "','" & txtbx_model.Text & "','" & cmb_peripheraltype.Text & "', 'Working')")
                strcon.Close()
                create("INSERT INTO tbl_inventorystorage (unit_id,serial_number,brand_name,model,unit_type,unit_condition) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "','" & cmb_brand.Text & "','" & txtbx_model.Text & "','Networking','Working')")
                strcon.Close()
                MessageBox.Show(cmb_peripheraltype.Text + " added sucessfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                reload("SELECT * FROM tbl_stocknetworkdevice", dgv_stockunit)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
    ...
    '        GetDateDiff()
    '        strcon.Close()
    '        updates("UPDATE tbl_stockperipherals SET brand_name = '" & cmb_brand.Text & "', model = '" & txtbx_model.Text & "', type = '" & cmb_peripheraltype.Text & "', purchased_value = '" & txtb_purchasedvalue.Text & "', date_purchased = '" & dtp_purchased.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
    ...
    '        updates("UPDATE tbl_peripheralcondition SET brand = '" & cmb_brand.Text & "', model = '" & txtbx_model.Text & "', type = '" & cmb_peripheraltype.Text & "', unit_condition = '" & cmb_unitcondition.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
    '        updates("UPDATE tbl_stockperipheralsvalue SET brand_name = '" & cmb_brand.Text & "', model = '" & txtbx_model.Text & "',type = '" & cmb_peripheraltype.Text & "', purchased_value = '" & txtb_purchasedvalue.Text & "' , depreciation_value = '" & txtb_depreciatedvalue.Text & "', date_purchased = '" & dtp_purchased.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
    '        strcon.Close()
    '        MessageBox.Show("Data updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
    '        reload("SELECT * FROM tbl_stockperipherals", dgv_stockunit)
    '    Catch ex As Exception
    '        MessageBox.Show(ex.Message)
    '    Finally
    '        strcon.Close()
    ...
    Public Sub FilterData(valueToSearch As String)
        ...
        If cmb_searchfilter.Text = "SERIAL NO." Then
            Dim searchquery As String = "SELECT * FROM tbl_stocknetworkdevice WHERE CONCAT(serial_number) LIKE '%" & txtbx_search.Text & "%'"
        ...
        ElseIf cmb_searchfilter.Text = "BRAND" Then
            Dim searchquery As String = "SELECT * FROM tbl_stocknetworkdevice WHERE CONCAT(brand_name) LIKE '%" & txtbx_search.Text & "%'"
            Dim command As New MySqlCommand(searchquery, strcon)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable
        ...
        ElseIf cmb_searchfilter.Text = "MODEL" Then
            Dim searchquery As String = "SELECT * FROM tbl_stocknetworkdevice WHERE CONCAT(model) LIKE '%" & txtbx_search.Text & "%'"
            Dim command As New MySqlCommand(searchquery, strcon)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable
