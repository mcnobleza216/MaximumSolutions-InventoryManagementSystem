...
Public Class stock_memory
    ...
    Private Sub stock_memory_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        depreciateitems()
        strcon.Close()
        reload("SELECT * FROM tbl_stockmemory", dgv_stockunit)
        FilterData("")
        ...
        panelsettings.Size = panelsettings.MinimumSize
        ...
        Try
            ...
            cmd3.CommandText = "SELECT * FROM tbl_memoryversion"
            cmd3.Connection = strcon

            daa3.SelectCommand = cmd3
            daa3.Fill(dtt3)

            cmb_memoryversion.DataSource = dtt3
            cmb_memoryversion.ValueMember = "version_id"
            cmb_memoryversion.DisplayMember = "version"
            cmb_memoryversion.Text = ""
    ...
    Private Sub btn_stockadduinit_Click(sender As Object, e As EventArgs) Handles btn_stockadduinit.Click
        ...
        cmb_memoryversion.Enabled = False
        txtb_memorysize.Enabled = False
        dgv_stockunit.Enabled = False
        txtbx_addunit.Show()
        txtbx_serialno.Hide()
        lbl_unitid.Text = "-------"
        dtp_purchased.Text = Date.Today
        btn_clear.Enabled = False
        ' cmb_storagetype.Enabled = False
    ...
    Private Sub dgv_stockunit_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_stockunit.CellContentClick
        ...
        ' cmb_storagetype.Text = dgv_stockunit.CurrentRow.Cells(4).Value
        ' txtb_storagesize.Text = dgv_stockunit.CurrentRow.Cells(5).Value
        ' txtb_purchasedvalue.Text = dgv_stockunit.CurrentRow.Cells(6).Value
    ...
    Private Sub Btn_Add2_Click(sender As Object, e As EventArgs) Handles Btn_Add2.Click
        ...
        storage = Convert.ToInt32(txtb_memorysize.Text)
        If (lbl_unitid.Text = "" Or txtbx_addunit.Text = "" Or cmb_brand.Text = "" Or txtb_purchasedvalue.Text = "" Or dtp_purchased.Text = "") Then
            MessageBox.Show("All information must be provided!", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        ElseIf storage > 256 Or storage < 2 Then
            MessageBox.Show("Storage size seems incorrect, please enter a valid storage size", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        Else
            strcon.Close()
            AddUnit()
            txtbx_addunit.Text = ""
    ...
    Public Sub EnableText()
        ...
        cmb_memoryversion.Enabled = True
        txtb_memorysize.Enabled = True
    ...
    Public Sub CMBPlaceHolder()
        ...
        cmb_memoryversion.Text = "Please Select"
    End Sub
    ...
    Public Sub getunitcondition()
        Try
            strcon.Open()
            cmd.CommandText = "SELECT unit_condition from tbl_memorycondition WHERE serial_number = '" & txtbx_serialno.Text & "'"
            cmd.Connection = strcon
        ...
        Try
            ...
            Else
                ...
                If dr.HasRows Then
                    ...
                    cmb_memoryversion.Text = dr.GetValue(4).ToString()
                    txtb_memorysize.Text = dr.GetValue(5).ToString()
                    txtb_purchasedvalue.Text = dr.GetValue(6).ToString()
                    '  dtp_purchased.Text = dr.GetValue(7).ToString()
                    ...
                    Else
                        ...
                        cmdd.CommandText = "SELECT depreciation_value FROM tbl_stockmemoryvalue WHERE serial_number = '" & txtbx_serialno.Text & "'"
    ...
    Public Sub AddUnit()
        ...
        Try
            ...
            Else
                ...
                GetDateDiff()
                strcon.Close()
                create("INSERT INTO tbl_stockmemory (unit_id,serial_number,brand_name,model,type,size,purchased_value,date_purchased) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','RAM','" & cmb_memoryversion.Text & "','" & txtb_memorysize.Text & "','" & txtb_purchasedvalue.Text & "','" & dtp_purchased.Text & "')")
                strcon.Close()
                create("INSERT INTO tbl_allstocksvalue (unit_id,serial_number,brand_name,model,unit_type,purchased_value,depreciation_value,date_purchased) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','RAM','Memory','" & txtb_purchasedvalue.Text & "','" & txtb_depreciatedvalue.Text & "','" & dtp_purchased.Text & "')")
                strcon.Close()
                create("INSERT INTO tbl_allstocks (unit_id,serial_number,brand_name,model,unit_type,unit_condition,availability,date_added) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','RAM','Memory','Working','Available','" & lbl_datetoday.Text & "')")
                strcon.Close()
                create("INSERT INTO tbl_memorycondition (unit_id,serial_number,brand,model,unit_condition) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "','" & cmb_brand.Text & "','RAM', 'Working')")
                strcon.Close()
                create("INSERT INTO tbl_inventorystorage (unit_id,serial_number,brand_name,model,unit_type,unit_condition) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "','" & cmb_brand.Text & "','RAM','Memory','Working')")
                strcon.Close()
                MessageBox.Show("Memory added sucessfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                reload("SELECT * FROM tbl_stockmemory", dgv_stockunit)
                strcon.Close()
            End If
        Catch ex As Exception
    ...
    'UPDATE UNIT FUNVTION'
    Private Sub UpdateUnit()
        Try
            strcon.Close()
            GetDateDiff()
            strcon.Close()
            updates("UPDATE tbl_stockmemory SET brand_name = '" & cmb_brand.Text & "', model = '" & txtbx_model.Text & "', type = '" & cmb_memoryversion.Text & "', size = '" & txtb_memorysize.Text & "', purchased_value = '" & txtb_purchasedvalue.Text & "', date_purchased = '" & dtp_purchased.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
            ...
            updates("UPDATE tbl_memorycondition SET brand = '" & cmb_brand.Text & "', model = '" & txtbx_model.Text & "', unit_condition = '" & cmb_unitcondition.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
            updates("UPDATE tbl_stockmemoryvalue SET brand_name = '" & cmb_brand.Text & "', model = '" & txtbx_model.Text & "', purchased_value = '" & txtb_purchasedvalue.Text & "' , depreciation_value = '" & txtb_depreciatedvalue.Text & "', date_purchased = '" & dtp_purchased.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
            strcon.Close()
    ...
    Public Sub FilterData(valueToSearch As String)
        ...
        If cmb_searchfilter.Text = "SERIAL NO." Then
            Dim searchquery As String = "SELECT * FROM tbl_stockmemory WHERE CONCAT(serial_number) LIKE '%" & txtbx_search.Text & "%'"
        ...
        ElseIf cmb_searchfilter.Text = "BRAND" Then
            Dim searchquery As String = "SELECT * FROM tbl_stockmemory WHERE CONCAT(brand_name) LIKE '%" & txtbx_search.Text & "%'"
            Dim command As New MySqlCommand(searchquery, strcon)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable
        ...
        ElseIf cmb_searchfilter.Text = "MODEL" Then
            Dim searchquery As String = "SELECT * FROM tbl_stockmemory WHERE CONCAT(model) LIKE '%" & txtbx_search.Text & "%'"
            Dim command As New MySqlCommand(searchquery, strcon)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable
    ...
    Private Sub cmb_memoryversion_KeyPress(ByVal sender As Object, e As KeyPressEventArgs) Handles cmb_memoryversion.KeyPress
        e.Handled = True
    End Sub

    Private Sub txtb_memorysize_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtb_memorysize.KeyPress
        If Not Char.IsNumber(e.KeyChar) AndAlso Not Char.IsControl(e.KeyChar) Then
            e.Handled = True
        End If
    End Sub

    Private Sub txtb_memorysize_TextChanged(sender As Object, e As EventArgs) Handles txtb_memorysize.TextChanged
        Dim digitsOnly As Regex = New Regex("[^\d]")
        txtb_memorysize.Text = digitsOnly.Replace(txtb_memorysize.Text, "")
