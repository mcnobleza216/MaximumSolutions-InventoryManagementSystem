...
Public Class stock_laptop
    ...
    Private Sub stock_laptop_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        depreciateitems()
        reload("SELECT * FROM tbl_stocklaptop", dgv_stockunit)
        FilterData("")
    ...
    Public Sub getunitcondition()
        Try
            strcon.Open()
            cmd.CommandText = "SELECT unit_condition from tbl_laptopcondition WHERE serial_number = '" & txtbx_serialno.Text & "'"
            cmd.Connection = strcon
            ...
            Else
                ...
                If dr.HasRows Then
                    ...
                    Else
                        ...
                        cmdd.CommandText = "SELECT depreciation_value FROM tbl_stocklaptopvalue WHERE serial_number = '" & txtbx_serialno.Text & "'"
                        cmdd.Connection = strcon
    ...
    Public Sub AddUnit()
        Try
            ...
            Else
                ...
                strcon.Close()
                create("INSERT INTO tbl_allstocks (unit_id,serial_number,brand_name,model,unit_type,unit_condition,availability,date_added) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "', '" & cmb_brand.Text & "','" & txtbx_model.Text & "','Laptop','Working','Available','" & lbl_datetoday.Text & "')")
                strcon.Close()
                create("INSERT INTO tbl_laptopcondition (unit_id,serial_number,brand,model,unit_condition) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "','" & cmb_brand.Text & "','" & txtbx_model.Text & "', 'Working')")
                strcon.Close()
                create("INSERT INTO tbl_inventorystorage (unit_id,serial_number,brand_name,model,unit_type,unit_condition) VALUES ('" & lbl_unitid.Text & "','" & txtbx_addunit.Text & "','" & cmb_brand.Text & "','" & txtbx_model.Text & "','Laptop','Working')")
                strcon.Close()
                MessageBox.Show("Laptop sucessfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                reload("SELECT * FROM tbl_stocklaptop", dgv_stockunit)
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message)
    ...
    Private Sub UpdateUnit()
        Try
            ...
            updates("UPDATE tbl_laptopcondition SET brand = '" & cmb_brand.Text & "', model = '" & txtbx_model.Text & "', unit_condition = '" & cmb_unitcondition.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
            updates("UPDATE tbl_stocklaptopvalue SET brand_name = '" & cmb_brand.Text & "', model = '" & txtbx_model.Text & "', purchased_value = '" & txtb_purchasedvalue.Text & "' , depreciation_value = '" & txtb_depreciatedvalue.Text & "', date_purchased = '" & dtp_purchased.Text & "' WHERE serial_number = '" & txtbx_serialno.Text & "'")
            strcon.Close()
    ...
    Public Sub FilterData(valueToSearch As String)
        ...
        If cmb_searchfilter.Text = "SERIAL NO." Then
            Dim searchquery As String = "SELECT * FROM tbl_stocklaptop WHERE CONCAT(serial_number) LIKE '%" & txtbx_search.Text & "%'"
        ...
        ElseIf cmb_searchfilter.Text = "BRAND" Then
            Dim searchquery As String = "SELECT * FROM tbl_stocklaptop WHERE CONCAT(brand_name) LIKE '%" & txtbx_search.Text & "%'"
            Dim command As New MySqlCommand(searchquery, strcon)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable
        ...
        ElseIf cmb_searchfilter.Text = "MODEL" Then
            Dim searchquery As String = "SELECT * FROM tbl_stocklaptop WHERE CONCAT(model) LIKE '%" & txtbx_search.Text & "%'"
            Dim command As New MySqlCommand(searchquery, strcon)
            Dim adapter As New MySqlDataAdapter(command)
            Dim table As New DataTable
    ...
    Private Sub dtp_purchased_ValueChanged(sender As Object, e As EventArgs) Handles dtp_purchased.ValueChanged
        ' dtp_purchased.CustomFormat = "yyyy-MM-dd"
    End Sub
