...
Public Class assign_internal
    ...
    Private Sub assign_internal_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        depreciateitems()
        reload("SELECT * FROM tbl_inventorystorage", dgv_storage)
        reload("SELECT * FROM tbl_employee", dgv_employee)
    ...
    Private Sub dgv_employee_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_employee.CellContentClick
        If lbl_unittype.Text = "System unit" Then
            Try
                ...
                If dr.HasRows Then
                    Dim result As DialogResult = MessageBox.Show("This employee already has an assigned system unit, are you sure you want to assign another system unit to this employee?", "NOTICE", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                    If result = DialogResult.Yes Then
                        ...
                        txtb_department.Text = dgv_employee.CurrentRow.Cells(5).Value
                        strcon.Close()
                    End If
                End If
                strcon.Close()
    ...
    Public Sub ASSIGNED()
        Try
            ...
            Else
                ...
                create("INSERT INTO tbl_unitdesignation (assigned_no,unit_id,serial_number,model,unit_type,assigned_to,employee_no,date_assigned,designation_type) VALUES('" & lbl_assignedno.Text & "','" & lbl_unitid.Text & "','" & txtb_serialno.Text & "','" & txtb_model.Text & "','" & lbl_unittype.Text & "','" & fullname & "','" & lbl_employeeno.Text & "','" & lbl_datetoday.Text & "','Internal')")
                create("INSERT INTO tbl_designations (assigned_no,unit_id,serial_number,model,unit_type,assigned_to,employee_no,date_assigned,designation_type) VALUES('" & lbl_assignedno.Text & "','" & lbl_unitid.Text & "','" & txtb_serialno.Text & "','" & txtb_model.Text & "','" & lbl_unittype.Text & "','" & fullname & "','" & lbl_employeeno.Text & "','" & lbl_datetoday.Text & "','Internal')")
                'MARKEDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD'
                create("INSERT INTO tbl_designateinternal (assigned_no,unit_id,serial_number,brand_name,model,unit_type,assigned_to,employee_no,department,date_assigned) VALUES('" & lbl_assignedno.Text & "','" & lbl_unitid.Text & "','" & txtb_serialno.Text & "','" & txtb_brand.Text & "','" & txtb_model.Text & "','" & lbl_unittype.Text & "','" & fullname & "','" & lbl_employeeno.Text & "','" & txtb_department.Text & "','" & lbl_datetoday.Text & "')")
                strcon.Close()
    ...
    Private Sub ComboBox2_TextChanged(sender As Object, e As EventArgs) Handles ComboBox2.TextChanged
        If ComboBox2.Text = "SELECT DEPARTMENT" Then
            reload("SELECT * FROM tbl_employee", dgv_employee)
        Else
            reload("SELECT * FROM tbl_employee WHERE department = '" & ComboBox2.Text & "'", dgv_employee)
        End If
    ...
    Public Sub FilterDataEmployee(valueToSearch As String)
        ...
        Dim searchquery As String = "SELECT * FROM tbl_employee WHERE CONCAT(employee_no,first_name,middle_name,last_name,suffix,department) LIKE '%" & txtb_searchstorage.Text & "%'"
        Dim command As New MySqlCommand(searchquery, strcon)
        Dim adapter As New MySqlDataAdapter(command)
        Dim table As New DataTable
    ...
    Private Sub txtb_serialno_TextChanged(sender As Object, e As EventArgs) Handles txtb_serialno.TextChanged
        If txtb_serialno.Text = "" Then
